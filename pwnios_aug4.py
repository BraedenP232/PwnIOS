...
class PwnIOS(plugins.Plugin):
        ...
        try:
            if self.agent:
                ...
                if face_elem and status_elem:
                    face = face_elem if isinstance(face_elem, str) else getattr(face_elem, 'value', 'AWAKE')
                    status = status_elem if isinstance(status_elem, str) else getattr(status_elem, 'value', 'ready')

                    face_name = os.path.basename(face).split('.')[0].strip().upper()
                    status = status.strip()
                ...
                face_name = self._state_to_face_mapping().get(state, 'AWAKE')
    ...
    def _get_battery_info(self):
        try:
            ...
            logging.error(f"[PwnIOS] Battery info error: {e}")
            return "N/A"
    ...
    def _get_temperature(self):
        try:
            ...
            logging.error(f"[PwnIOS] Temp error: {e}")
        return "N/A"
    ...
    def _get_face_image(self, face_name):
        ...
        # self._log_available_faces(face_name, face_variations)
        return None

    def _log_available_faces(self, face_name, face_variations):
        base_paths = ["/custom-faces/", "/etc/pwnagotchi/faces/", "/home/pi/custom-faces/"]
        for base_path in base_paths:
            try:
                available_faces = os.listdir(base_path)
                logging.warning(f"[PwnIOS] Face '{face_name}' not found. Available faces: {available_faces}")
                logging.warning(f"[PwnIOS] Tried variations: {face_variations}")
            except Exception as e:
                logging.error(f"[PwnIOS] Could not list faces directory: {e}")

    def _broadcast_status_change(self, status):
        face, _ = self._get_current_face_and_status()
        self.queue_message({
            "type": "status_change", 
            "data": {"status": status, "face": face}
        })
